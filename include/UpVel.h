...
public:
  ...
  static void Update_Vel(Population& pop) {
    ...
    double dumping = 2.0/TMath::Abs(2.0-phi-TMath::Power(TMath::Power(phi,2) - 4.0*phi,0.5));
    ...
    for (size_t i=0; i<pop.Size(); ++i){
      for (size_t j=0; j<pop.AccessParticle(i).Size(); ++j){
        ...
        if (dumping*(pop.AccessParticle(i).ViewVelocity(j) + cognitive + social) < pop.Configuration().Vmax){
          pop.AccessParticle(i).SetVelocity(j, dumping*(pop.AccessParticle(i).ViewVelocity(j) + cognitive + social));
        } else {
